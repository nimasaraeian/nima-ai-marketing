"""
Image extractor: converts image bytes into PageMap schema using vision API.
"""
import logging
import json
import os
...
async def extract_from_image(image_bytes: bytes, goal: str) -> PageMap:
    """
    Extract PageMap from image using OpenAI Vision.
    ...
    system_prompt = """You are a page analysis expert. Extract structured information from this landing page screenshot.
...
- offer: Pricing, discount, trial info if visible
...
- visual_hierarchy: Visual focus element, CTA visibility (0-1), noise/clutter (0-1)
...
Return ONLY the JSON object, no markdown, no explanation."""
    ...
    user_prompt = "Extract all page information and return as JSON."
    ...
    try:
        ...
        logger.error(f"Failed to parse OpenAI Vision JSON: {e}")
        raise ValueError(f"Invalid JSON from vision API: {e}")
